  ...
  try {
    ...
    if (resend) {
      ...
      await resend.emails.send({
        from,
        to: email,
        subject: "You're on the list. Leous Drop 001.",
        html: `
          <div style="background:#080808;color:#F0EDE8;font-family:Arial,sans-serif;padding:40px;max-width:500px;margin:0 auto;">
            <p style="font-size:12px;letter-spacing:0.15em;color:#6B6B6B;margin-bottom:24px;">Leous</p>
            ...
            <p style="color:#6B6B6B;font-size:11px;">&copy; 2026 Leous — Bangalore</p>
          </div>
        `,
      }).catch((e) => {
    ...
    console.error('Waitlist API general error:', err)
